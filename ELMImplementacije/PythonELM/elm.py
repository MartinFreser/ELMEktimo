...
from ELMImplementacije.PythonELM.random_layer import RandomLayer, MLPRandomLayer
# from ELMimplementacije.PythonELM.random_layer import RandomLayer, MLPRandomLayer
...
class GenELMClassifier(BaseELM, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        self.rawPreds = raw_predictions[:,0]
        return class_predictions
    def predict_proba(self,X):
        """Predict probabilty values between -1 and 1 using the model
        ...
        """
        eps = 10e-15
        raw_predictions = self.decision_function(X)[:,0]
        probs = np.array([-1+eps if pred < -1 else pred if pred<1 else 1-eps for pred in raw_predictions])
        probs = [[1-prob,prob] for prob in (probs+1)/2]
...
class ELMClassifier(ELMRegressor):
    ...
    def predict_proba(self,X):
        ...
        """
        raw_predictions = self.decision_function(X)[:,0]
        probs = np.array([-1 if pred < -1 else pred if pred<1 else 1 for pred in raw_predictions])
        probs = [[1-prob,prob] for prob in (probs+1)/2]
        return np.array(probs)
